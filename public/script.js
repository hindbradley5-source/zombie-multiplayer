...
// Bug Fix 1: separate dash key from move keys, and track mouse down state
let keys = { up: false, down: false, left: false, right: false, dash: false };
let mouseX = canvas.width / 2;  // Bug Fix 2: initialize to center, not 0/0 (avoids first-frame shoot-to-corner)
let mouseY = canvas.height / 2;
let isMouseDown = false;         // Bug Fix 3: track hold-to-fire for desktop
let pulseTick = 0;
let fxParticles = [];
let autoFireEnabled = false;
let lastAutoFireTime = 0;

// Bug Fix 4: flag to suppress game input when a UI overlay is open
function isUIOpen() {
...
let targetCamX = 1600;
let targetCamY = 1600;
let cameraInitialized = false;  // Bug Fix 5: lock camera snap on game start
...
// Bug Fix 6: right-joystick fire rate limiter (prevents socket flooding on mobile)
let lastRightJoyFireTime = 0;
const RIGHT_JOY_FIRE_INTERVAL = 180; // ms
...
window.addEventListener('touchmove', (e) => {
    e.preventDefault(); // Bug Fix 11: prevent default scroll/zoom on all touch moves
    for (let i = 0; i < e.changedTouches.length; i++) {
        ...
        if (touch.identifier === leftTouchId) {
            ...
            // Bug Fix 6: rate-limit right joystick fire
            const now = Date.now();
...
socket.on('stateUpdate', (state) => {
    ...
    if (me) {
        ...
        const worldX = mouseX - canvas.width/2 + (localPlayerX + me.size/2);
        const worldY = mouseY - canvas.height/2 + (localPlayerY + me.size/2);
        // Bug Fix 14: compute aimAngle from mouse, don't use stale server aimAngle
        const aimAngle = Math.atan2(worldY - (localPlayerY + me.size/2), worldX - (localPlayerX + me.size/2));
...
function drawDetailedPlayer(ctx, p) {
    ...
    // Bug Fix: use live mouse-derived aimAngle for local player rendering
    const angle = isMe
        ? Math.atan2(
...
function render(nowTime) {
    ...
    // 🎥 60 FPS Smooth Camera Lerping — use tighter lerp for responsiveness
